/obj/item/mecha_parts/mecha_equipment/tool/sleeper
	...
	Topic(href,href_list)
		...
		if(mecha_filter.get("eject"))
			go_out()
		if(mecha_filter.get("view_stats"))
			chassis.occupant << browse(get_occupant_stats(),"window=msleeper")
			onclose(chassis.occupant, "msleeper")
			return
		if(mecha_filter.get("inject"))
			inject_reagent(mecha_filter.getType("inject",/datum/reagent),mecha_filter.getObj("source"))
		return
...
/obj/item/mecha_parts/mecha_equipment/tool/syringe_gun
	...
	Topic(href,href_list)
		...
		if(mecha_filter.get("toggle_mode"))
			mode = !mode
			update_equip_info()
			return
		if(mecha_filter.get("select_reagents"))
			...
			for(var/i=1 to known_reagents.len)
				if(m>=synth_speed)
					break
				var/reagent = mecha_filter.get("reagent_[i]")
			...
			return
		if(mecha_filter.get("show_reagents"))
			chassis.occupant << browse(get_reagents_page(),"window=msyringegun")
		if(mecha_filter.get("purge_reagent"))
			var/reagent = mecha_filter.get("purge_reagent")
			if(reagent)
				reagents.del_reagent(reagent)
			return
		if(mecha_filter.get("purge_all"))
